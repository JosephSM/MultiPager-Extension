...
let mylastTime = -1
vid.addEventListener("timeupdate", (e) => {
    mycurrentTime = Math.floor(vid.currentTime)
    if(mycurrentTime in highlighters && mycurrentTime !== myLastTime){
...
navheight = canvas.offsetTop
canvasHeight = canvas.scrollHeight
viewportHeight = window.innerHeight
dafHeight = 1268.8
highlighterOffset = currentHighlighter.offsetTop (the actual offset on the daf)
canvasSCrollPosition = $("#canvasHolder").scrollTop()
bodyScroll
...
var myf = () => {
    ...
    if (currentY-top < 0) {
      $("#canvasHolder").animate({scrollTop:0});
    }
    else if (620 + top - currentY < 100) {
      if (delta >= 620-maxY) {
        $("#canvasHolder").animate({scrollTop:-delta});
      }
      else {
